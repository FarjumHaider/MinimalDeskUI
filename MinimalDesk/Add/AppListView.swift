...
struct AppListView: View {
    ...
    @State var showCustomApp = false
    @State var showCustomAppAlert = false
    ...
    func appList() -> some View {
        List(searchResults, id: \.self) { name in
            ...
            .onTapGesture {
                ...
                if restrictedApps.contains(name) {
                    showCustomAppAlert = true
                }
                else {
                    onSelectingApp(name)
                }
        ...
        .alert(isPresented: $showCustomAppAlert) {
            Alert(
                title: Text("System App Restricted"),
                message: Text("Please add this app using URL Schemes"),
                primaryButton: .default(Text("OK")) {
                    showCustomApp  = true
                },
                secondaryButton: .cancel()
            )
        } .fullScreenCover(isPresented: $showCustomApp) {
            NavigationStack {
                CustomAppView()
            }
        }
