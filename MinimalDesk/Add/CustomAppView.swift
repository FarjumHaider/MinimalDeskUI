...
struct CustomAppView: View {
    ...
    private var placeHolderUrlScheme: String {
        guard scheme.isEmpty else { return scheme }
        guard appName.isEmpty == false else { return "URL Scheme" }
        return "\(appName.lowercased())://"
    }

    private var placeHolderShortcut: String {
        guard shortcut.isEmpty else { return shortcut }
        guard appName.isEmpty == false else { return "Shortcut" }
        return appName
    }
    ...
    private var appRank: Int {
        -1 * viewModel.customAppList.count - 1
    }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 16) {
                ...
                // MARK: - PICKER
                Picker("", selection: $deepLinkType) {
                    Text("URL Scheme").tag(DeepLinkType.urlScheme)
                    Text("Shortcuts").tag(DeepLinkType.shortcut)
                }
                .pickerStyle(.segmented)
                .clipShape(RoundedRectangle(cornerRadius: 9))
                .padding(.horizontal)
...
//                        Spacer()
//
//                        NavigationLink(destination: PDFViewerView(pdfName: "instructions")) {
//                            Text("How to Guide")
//                                .font(Font.custom("Inter", size: 16))
...
//                    }
                    ...
                    HStack {
                        ...
                        NavigationLink {
                            PDFViewerView(pdfName: "instructions")
                        } label: {
                            Text("How to Guide")
                                .font(Font.custom("Inter", size: 16))
                                .foregroundStyle(Color.blue)
                ...
                // MARK: - CUSTOM APPS LIST
                if !viewModel.customAppList.isEmpty {
                    VStack(alignment: .leading) {
                        ForEach(0..<viewModel.customAppList.count, id: \.self) { index in
                            VStack {
                                HStack {
                                ...
                                if index != viewModel.customAppList.count - 1 {
                                    Divider().background(Color(rgbRed: 0x2f, green: 0x32, blue: 0x3c))
                                }
                            }
                        }
    ...
    // MARK: - Test Custom App
    private func testCustomApp() {
        let shortcutName = (shortcut.isEmpty ? appName : shortcut)
        let finalName = shortcutName.isEmpty ? "My App Shortcut" : shortcutName

        guard let encodedName = finalName.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              let shortcutURL = URL(string: "shortcuts://create-shortcut?name=\(encodedName)") else {
            showingAlert = true
            alertMessage = "Invalid shortcut name."
            return
        }
        ...
        if UIApplication.shared.canOpenURL(shortcutURL) {
            ...
            showingAlert = true
            alertMessage = "Shortcuts app not available."
        }
    }
}
