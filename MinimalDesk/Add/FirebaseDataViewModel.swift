...
extension FirebaseDataViewModel {
    func fetchAppNames(completion: @escaping ([String]) -> Void) {
        dbRef.getDocuments { snapshot, error in
            guard error == nil else {
                print("Error fetching app names: \(String(describing: error))")
                completion([])
                return
            }
            
            let names = snapshot?.documents.compactMap { doc in
                doc.data()["appName"] as? String
            } ?? []
            
            DispatchQueue.main.async {
                self.onlyAppName = names
